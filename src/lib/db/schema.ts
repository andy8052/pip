...
export const launches = pgTable(
  ...
  (table) => [
    ...
    index("launches_target_twitter_idx").on(table.targetTwitterUsername),
    index("launches_launcher_user_idx").on(table.launcherUserId),
...
export const feeCollections = pgTable(
  ...
  (table) => [
    index("fee_collections_launch_idx").on(table.launchId),
  ]
);
